...
def update_config():
    ...
    try:
        ...
        # Validate inputs
        if not api_url:
            return render_template('index.html', error="API URL is required", config=load_config())
        
        if interval_seconds < 1:
            return render_template('index.html', error="Interval must be at least 1 second", config=load_config())
            
        if not 0 <= failure_rate <= 100:
            return render_template('index.html', error="Failure rate must be between 0 and 100", config=load_config())
            
        if not failure_modes and failure_rate > 0:
            return render_template('index.html', error="Select at least one failure mode if failure rate > 0", config=load_config())
        ...
        if save_config(new_config):
            ...
            return render_template('index.html', error="Failed to save configuration", config=load_config())
            
    except Exception as e:
        logger.error(f"Error updating config: {e}")
        return render_template('index.html', error=str(e), config=load_config())
