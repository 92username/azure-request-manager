...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/requester.log")],
)
...
API_UP = Gauge("azure_api_up", "Indica se a API do Azure está respondendo normalmente")
...
def make_request():
    ...
    try:
        ...
        if response.status_code >= 400:
            ...
            logger.info(
                f"Requisição bem-sucedida: {status} - {url} ({request_time:.2f}s)"
            )
        ...
        ERROR_COUNT.labels(
            method=method, endpoint=endpoint, error_type="connection_error"
        ).inc()
        REQUEST_COUNT.labels(
            method=method, endpoint=endpoint, status="connection_error"
        ).inc()

    except Exception as e:
        logger.error(f"Erro inesperado: {url} - {e}")
        ERROR_COUNT.labels(
            method=method, endpoint=endpoint, error_type="unexpected_error"
        ).inc()
...
def main():
    ...
    logger.info("Servidor de métricas iniciado na porta 8001")
